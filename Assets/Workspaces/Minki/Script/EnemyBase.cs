...
public abstract class EnemyBase : MonoBehaviour
{
    ...
    {
        // 현재 머리 위에 떠 있는 의도(isNextAttack)를 기준으로 정확히 그에 맞는 행동 수행
        if (isNextAttack)
        {
    ...
    // 공격 모션 수행 래퍼: 모션이 "완전히 끝난 뒤에" 다음 의도를 랜덤으로 결정하고 아이콘 갱신
    IEnumerator AttackRoutineWrapper()
    {
        isActing = true;
        yield return StartCoroutine(PlayCustomAttack()); // 공격 모션 끝날 때까지 대기
        
        // [핵심] 모션이 완전히 끝나고 제자리로 돌아온 시점에 다음 턴 의도를 [무작위]로 결정하고 아이콘 바꿈
        isNextAttack = (Random.value > 0.5f); 
        UpdateIntentUI();
    ...
    // 방어 모션 수행 래퍼: 모션이 "완전히 끝난 뒤에" 다음 의도를 랜덤으로 결정하고 아이콘 갱신
    IEnumerator DefendRoutineWrapper()
    {
        isActing = true;
        yield return StartCoroutine(PlayCustomDefend()); // 방어 모션 끝날 때까지 대기
